...
:configure
call "%CONDA_CMD%" run -n workflow-c python -c "import PIL, tkinter, sys, numpy, openpyxl, pandas; raise SystemExit(sys.version_info[:2] != (3, 11))" >nul 2>nul
...
:verify
call "%CONDA_CMD%" run -n workflow-c python -c "import PIL, tkinter, sys, numpy, openpyxl, pandas; raise SystemExit(sys.version_info[:2] != (3, 11))"
if errorlevel 1 goto :failed
echo.
