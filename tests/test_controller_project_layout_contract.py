...
class ControllerProjectLayoutContractTests(unittest.TestCase):
    def test_layout_remains_opt_in_and_reports_folder_move(self) -> None:
        ...
        self.assertIn(
            "selected image-root folder itself will be moved intact into Raw", text
        )
        ...
        self.assertLess(
            block.index("messagebox.askyesno("),
            block.index("project_layout.initialize_project("),
        )
        self.assertIn("Metadata: {planned.metadata_dir}", block)

    def test_layout_configures_output_paths_and_rebases_csvs_moved_with_source(
        self,
    ) -> None:
        text = CONTROLLER.read_text(encoding="utf-8")
        ...
        self.assertIn(
            'self.vars["matrix_output"].set(str(layout.matrix_output))', block
        )
        self.assertIn("project_layout.rebase_moved_path", block)
        self.assertIn(
            "candidate_dirs = [layout.metadata_dir, layout.project_root]", block
        )
    ...
    def test_existing_raw_layout_is_recognized_without_another_move(self) -> None:
        ...
        self.assertIn(
            "moved_from = None if layout.image_root == source else source", text
        )
    ...
    def test_completed_v10_applets_are_optional_from_stable_controller(self) -> None:
        text = CONTROLLER.read_text(encoding="utf-8")
        self.assertIn('text="Open V10 project applets"', text)
        self.assertIn('self.launch_python("tools/workflow_applets_gui.py")', text)
