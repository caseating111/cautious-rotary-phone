...
class PreflightBatchTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.grid_csv.write_text("Experiment,Set,GridCols,Column,Strain\nE1,A,2,1,WT\nE1,A,2,2,mut1\n", encoding="utf-8")
        self.images_csv.write_text("Filename,Experiment,Set,Type\nplate1.jpg,E1,A,YPDA\n", encoding="utf-8")
        self.config = {"image_root": str(self.image_root), "crop_output": str(self.crop_root), "grid_csv": str(self.grid_csv), "images_csv": str(self.images_csv), "crop_width": 130, "crop_height": 546}
    ...
    def expected_names(self) -> list[str]:
        ...
        grid_rows = [{"Column": "1", "Strain": "WT"}, {"Column": "2", "Strain": "mut1"}]
    ...
    def test_preflight_semantic_validation_failure_is_blocking(self) -> None:
        ...
        validator.write_text("print('synthetic preflight metadata failure')\nraise SystemExit(1)\n", encoding="utf-8")
        config = {"grid_csv": str(self.grid_csv), "images_csv": str(self.images_csv), "condition_order_csv": str(self.root / "condition_order.csv")}
    ...
    def test_exact_existing_outputs_mark_image_complete(self) -> None:
        output_dir = self.crop_root / "setA"; output_dir.mkdir()
        source_mtime = (self.source_folder / "plate1.jpg").stat().st_mtime_ns
        for index, name in enumerate(self.expected_names(), 1):
            path = output_dir / name; self.save_expected_png(path); os.utime(path, ns=(source_mtime + index, source_mtime + index))
        lines, problems, pending = build_report(self.config)
        self.assertFalse(problems); self.assertEqual(pending, [])
        self.assertIn("Already complete images: 1", lines); self.assertIn("Crops still to produce/rebuild: 0", lines)

    def test_rotated_existing_outputs_are_also_current(self) -> None:
        output_dir = self.crop_root / "setA"; output_dir.mkdir()
        source_mtime = (self.source_folder / "plate1.jpg").stat().st_mtime_ns
        for index, name in enumerate(self.expected_names(), 1):
            path = output_dir / name; self.save_expected_png(path, (546, 130)); os.utime(path, ns=(source_mtime + index, source_mtime + index))
        lines, problems, pending = build_report(self.config)
        self.assertFalse(problems); self.assertEqual(pending, []); self.assertIn("Already complete images: 1", lines)

    def test_wrong_dimension_expected_crop_marks_plate_pending(self) -> None:
        output_dir = self.crop_root / "setA"; output_dir.mkdir()
        source_mtime = (self.source_folder / "plate1.jpg").stat().st_mtime_ns
        for index, name in enumerate(self.expected_names(), 1):
            path = output_dir / name; self.save_expected_png(path, (130, 546) if index > 1 else (100, 100)); os.utime(path, ns=(source_mtime + index, source_mtime + index))
        lines, problems, pending = build_report(self.config)
        self.assertFalse(problems); self.assertEqual([row["Filename"] for row in pending], ["plate1.jpg"])
        self.assertIn("INCOMPATIBLE EXPECTED CROPS — WILL REBUILD (1)", lines); self.assertIn("Crops still to produce/rebuild: 1", lines)

    def test_source_newer_than_existing_outputs_marks_plate_pending_for_rebuild(self) -> None:
        output_dir = self.crop_root / "setA"; output_dir.mkdir()
        for name in self.expected_names(): self.save_expected_png(output_dir / name)
        ...
        lines, problems, pending = build_report(self.config)
        self.assertFalse(problems); self.assertEqual([row["Filename"] for row in pending], ["plate1.jpg"])
        self.assertIn("Already complete images: 0", lines); self.assertIn("Crops still to produce/rebuild: 4", lines); self.assertIn("STALE EXPECTED CROPS — WILL REBUILD (4)", lines)

    def test_partially_complete_plate_warns_about_whole_plate_rerun_without_blocking(self) -> None:
        output_dir = self.crop_root / "setA"; output_dir.mkdir()
        existing = output_dir / self.expected_names()[0]; self.save_expected_png(existing)
        source_mtime = (self.source_folder / "plate1.jpg").stat().st_mtime_ns; os.utime(existing, ns=(source_mtime + 1, source_mtime + 1))
        lines, problems, pending = build_report(self.config)
        self.assertFalse(problems); self.assertEqual([row["Filename"] for row in pending], ["plate1.jpg"])
        self.assertIn("PARTIALLY COMPLETE PLATES — NON-BLOCKING (1)", lines); self.assertIn("- setA/plate1.jpg: 1 current, 3 missing/stale/incompatible", lines)

    def test_unrelated_unexpected_crop_png_is_reported_but_not_blocking(self) -> None:
        output_dir = self.crop_root / "setA"; output_dir.mkdir(); (output_dir / "old_stale_crop.png").write_bytes(b"derived placeholder")
        lines, problems, pending = build_report(self.config)
        self.assertFalse(problems); self.assertEqual([row["Filename"] for row in pending], ["plate1.jpg"])
        self.assertIn("OTHER UNEXPECTED CROP PNGS — NON-BLOCKING (1)", lines); self.assertIn("- setA/old_stale_crop.png", lines)

    def test_old_strain_suffix_crop_is_classified_as_superseded_non_blocking(self) -> None:
        output_dir = self.crop_root / "old"; output_dir.mkdir(); superseded = output_dir / "E1_A_YPDA_01_Top_OLD_STRAIN.png"; self.save_expected_png(superseded, (546, 130))
        lines, problems, pending = build_report(self.config)
        self.assertFalse(problems); self.assertEqual([row["Filename"] for row in pending], ["plate1.jpg"])
        self.assertIn("SUPERSEDED PREFIX CROPS — NON-BLOCKING (1)", lines); self.assertIn("- old/E1_A_YPDA_01_Top_OLD_STRAIN.png", lines)
    ...
    def test_semicolon_in_source_folder_is_blocking_before_fiji_handoff(self) -> None:
        unsafe = self.image_root / "set;unsafe"; self.source_folder.rename(unsafe); self.source_folder = unsafe
        lines, problems, _ = build_report(self.config)
        self.assertTrue(problems); self.assertIn("SOURCE FOLDERS UNSAFE FOR FIJI ARGUMENT HANDOFF (1)", lines); self.assertIn("- set;unsafe", lines)

    def test_duplicate_source_basename_is_blocking(self) -> None:
        second_folder = self.image_root / "setB"; second_folder.mkdir(); (second_folder / "plate1.jpg").write_bytes(b"synthetic placeholder")
        lines, problems, _ = build_report(self.config)
        self.assertTrue(problems); self.assertIn("DUPLICATE SOURCE BASENAMES (1)", lines)
    ...
    def test_same_metadata_in_different_source_folders_is_downstream_ambiguous(self) -> None:
        second_folder = self.image_root / "setB"; second_folder.mkdir(); (second_folder / "plate2.jpg").write_bytes(b"synthetic placeholder")
        self.images_csv.write_text("Filename,Experiment,Set,Type\nplate1.jpg,E1,A,YPDA\nplate2.jpg,E1,A,YPDA\n", encoding="utf-8")
        lines, problems, pending = build_report(self.config)
        self.assertTrue(problems); self.assertEqual(sorted(row["Filename"] for row in pending), ["plate1.jpg", "plate2.jpg"])
    ...
    def test_distinct_type_in_different_source_folder_remains_unambiguous(self) -> None:
        ...
        lines, problems, pending = build_report(self.config)
        self.assertFalse(problems); self.assertEqual(sorted(row["Filename"] for row in pending), ["plate1.jpg", "plate2.jpg"])
