...
CONFIGURED_MACRO = APP_DIR / "batch_four_point.configured.ijm"
STATE_FILE = APP_DIR / "four_point_grid_registration.state.txt"
...
def validate_grid_widths(config: dict) -> None:
    ...
    if unsupported:
        raise SystemExit(
            "The established four-point route supports 10- or 12-column grids. "
...
def configure_source_settings(source: str, config: dict) -> str:
    replacements = {
        ...
        'stateFile  = "path here";': f'stateFile  = "{macro_path(STATE_FILE)}";',
...
def enhance_four_point_macro(source: str) -> str:
    ...
    if source.count(START_MARKER) != 1 or source.count(EXPORT_MARKER) != 1:
        raise SystemExit("Four-point calibration markers changed; refusing to guess where to patch.")

    start = source.index(START_MARKER)
    export = source.index(EXPORT_MARKER, start)
    block = r'''        // ====================================================
...
def main() -> None:
    ...
    parser.add_argument(
        "--prepare-only",
        action="store_true",
        help="validate, preflight, and build the configured four-point Fiji macro without launching Fiji",
    )
    ...
    subprocess.Popen([str(fiji), "--no-splash", "-macro", str(macro)], cwd=fiji.parent)
