...
def build_report(
    config: dict,
    require_grid_interpolation: bool = True,
    ...
    unsupported_interpolation_grids: list[str] = []
    if require_grid_interpolation:
        for (exp, set_name), rows_for_grid in sorted(grid_by_key.items()):
            ...
            if len(declared_values) == 1:
                ...
                if declared < 2:
                    unsupported_interpolation_grids.append(f"{exp}/{set_name}: GridCols={declared}")
    ...
    sections = [
        ("GRIDS UNSUPPORTED BY GRID INTERPOLATION", unsupported_interpolation_grids),
