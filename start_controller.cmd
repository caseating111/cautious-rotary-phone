...
if not errorlevel 1 (
    call conda run --no-capture-output -n cautious-rotary-phone python tools\workflow_controller.py
    if not errorlevel 1 exit /b 0
    echo.
    echo Named conda environment was unavailable or failed; trying Anaconda base.
    call conda run --no-capture-output -n base python tools\workflow_controller.py
    if not errorlevel 1 exit /b 0
    echo.
