...
The launcher supports `--prepare-only`. That route still performs CSV validation, batch preflight, pending-image generation, exact source-marker checks and configured macro construction, but does not start Fiji. It provides a cheap integration check before any interactive desktop validation.
