...
**ROI presets** refer to the small per-culture/grid-box size used by the existing ROI 1-Click route and full-grid QC geometry. They are not the tall manually positioned `1 / 2` and `2 / 2` alignment rectangles; those remain native ImageJ selections controlled by the user.
...
The single AHK helper recognizes both the `1 / 2` / `2 / 2` full-column dialogs and the preserved `1 / 4` through `4 / 4` fallback dialogs. It also moves newly created placement dialogs once to a predictable corner without activating them. `X` remains specific to full-column QC retry; `Esc` remains the explicit helper stop. Native ImageJ arrow/Alt-arrow ROI fine adjustment is intentionally left native rather than re-forwarded through AHK.
...
Pillow output jobs are run synchronously through `tools/run_existing_pillow_from_config.py`. That wrapper validates readiness, stages only exact current crop files in a disposable directory, normalizes orientation on the copies, and then runs the established Pillow composition script. The controller captures its exit status/output so missing/duplicate/incompatible crop errors are visible. Real `crop_output` images are not rotated or rewritten by final-output generation.
