...
- One V10-apps visual/QC session using local real images only after the privacy gate: create/open project state; preview/apply optional Working copies; confirm both line and two-point orientation, automatic preview, and automatic next-image movement; calibrate a near-1750-pixel crop on a roughly 2K source and confirm the displayed source/measured/proposed dimensions plus the `tk_canvas_image_item_to_source_pixels` route, no-margin round-down-50 default, one alternate saved margin/rounding rule, and automatic crop preview. With both displays connected, move the same window between the 4K and lower-resolution monitor and confirm stable measurements plus proportionate UI; repeat once with only the 4K monitor. Save two differently named crop sizes, switch between them in **Saved size preset**, reopen the project, and confirm the chosen preset remains active. Attach the saved grid; inspect visibility and annotation previews; export later Unprocessed and Processed Top/Low cultures; then compose one matrix mixing at least one Top and one Low crop. Confirm raw sources remain unchanged, transformed outputs retain the expected pixel dimensions as lossless PNGs, rejected previews write nothing, accepted outputs use numbered/provenance-recorded locations, and label/crop/matrix presentation is usable. Exercise the collapsed Hotkeys panel and verify `X/V/Z/C` consistency, stage-specific keys, and that shortcuts do not fire while typing or selecting batch/matrix rows. Report observations textually only.
