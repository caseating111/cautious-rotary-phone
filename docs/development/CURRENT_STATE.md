...
Keep the helper thin: Z advances/accepts recognized alignment dialogs, X selects Retry on either `Alignment QC` or `Full-grid QC`, Esc exits, placement dialogs go upper-left, and the visible Fiji toolbar is positioned upper-right. A bounded fifteen-second watch handles delayed Java dialog titles. Window Spy identifies the launcher overlay as `SunAwtWindow` from `fiji-windows-x64.exe`; the helper repeatedly removes its topmost state, lowers it and hides it by class/executable without closing either Fiji process or relying on PID/window IDs. It does not establish reliable Fiji ownership/control.
