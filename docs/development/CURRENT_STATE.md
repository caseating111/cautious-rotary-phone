...
Keep the helper thin: Z advances/accepts recognized alignment dialogs, X selects Retry on either `Alignment QC` or `Full-grid QC`, Esc exits, placement dialogs go upper-left, and the visible Fiji toolbar is positioned upper-right. A bounded ten-second watch handles delayed Java dialog titles. It removes topmost state and lowers broad `SunAwtWindow` utility windows from `fiji-windows-x64.exe` without hiding or closing them, then pins known macro dialogs and the `__alignment_view__` preview above them. The preview is placed below the dialog region to avoid overlap. It does not establish reliable Fiji ownership/control.
