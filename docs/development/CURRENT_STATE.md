...
- ROI preset reads now fall back safely when the saved preset value is missing, NaN or non-positive.
...
- `ahk/full_column_alignment_hotkeys.ah2` remains small global-hotkey convenience only. It now watches only the actual remaining dialogs (`1 / 2`, `2 / 2`, `Alignment QC`, `ALL DONE`); obsolete `Next plate` handling was removed.
- `tests/test_alignment_macro_contract.py` statically protects the manual-authority/seed/native-profile contract without introducing speculative headless Fiji CI.
...
- Batch preparation verifies the production macro, alignment macro, crop helper, validator and preflight helper exist. Actual standalone launch also verifies Fiji; prepare-only deliberately remains independent of Fiji installation.
...
- `tests/test_batch_interaction.py` protects the reduced-click interaction and Experiment/Set/Type context handoff.
...
- An expected crop counts as current only when it exists **and is not older than its source image**. Existing derived crops older than the source are listed under **STALE EXPECTED CROPS — WILL REBUILD** and the whole plate is returned to pending. This prevents a replaced/updated source image from silently reusing old crops with the same filenames.
...
- `tests/test_preflight_batch.py` covers semantic validation, source/crop tree separation, missing/complete/partial/stale-source states, stale unrelated PNG reporting, semicolon-unsafe folders, duplicate basenames, same-folder collisions and downstream ambiguities.
...
- **Missing expected logical crop cells are now blocking by default**, preventing final matrices from silently rendering blank cells because crop generation was incomplete. Intentional partial output is available only through explicit CLI `--allow-missing`; controller output remains strict.
...
- **Run full-column batch** now runs `run_full_column_batch_from_config.py --prepare-only` synchronously first. Only after validation/preflight/build succeeds does it verify Fiji, optionally start AHK, and launch the already-prepared macro directly. This prevents ordinary prep failures from leaving hotkeys running and removes duplicate preflight/build work.
...
- Current regression coverage includes preflight/resume/freshness, CSV validation, source adapters, Fiji launcher construction, output navigation, batch interaction, alignment macro contract and Pillow input completeness.
