...
Orientation supports either a dragged edge line or two clicked points, with automatic preview by default and saved last-used mode. On Windows the app enables Per-Monitor-v2 before creating Tk, then maps official canvas coordinates through the actual rendered-image scale so plate calibration receives original-image pixels at 100–200%+ display scaling. Its reusable rule defaults to no margin and rounding down to the nearest 50 pixels, with saved controls for disabled/down/nearest/up rounding, increment, and per-side pixel or percentage margin. A known exact final side may instead be accepted numerically without calibration clicks. Crop placement automatically previews by default. Accepted transformed orientation, whole-plate crop, and visibility files are lossless PNG derivatives at their calculated pixel dimensions; originals and skipped exact-copy outputs retain their source files/formats.
